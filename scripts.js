...
function updateStockChart(stockSymbol) {
    const encodedStock = encodeURIComponent(stockSymbol.split(".")[0]);
    const exchange = stockSymbol.endsWith(".NS") ? "NSE" : "BSE";
    stockChartContainer.innerHTML = `<div class="tradingview-widget-container">
        <div id="tradingview_${encodedStock}"></div>
        <script type="text/javascript">
        new TradingView.widget(
        {
            "width": "100%",
            "height": "400px",
            "symbol": "${exchange}:${encodedStock}",
            "interval": "D",
            ...
            "container_id": "tradingview_${encodedStock}"
        });
        </script>
    </div>`;
}
